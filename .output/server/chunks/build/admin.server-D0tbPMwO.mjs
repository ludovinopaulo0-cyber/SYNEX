import { Z as TSS_SERVER_FUNCTION } from "./server.mjs";
...
function fillPlaceholders(params, values) {
  return params.map((p2) => {
    if (is(p2, Placeholder)) {
      if (!(p2.name in values)) throw new Error(`No value for placeholder "${p2.name}" was provided`);
      return values[p2.name];
    }
    if (is(p2, Param) && is(p2.value, Placeholder)) {
      if (!(p2.value.name in values)) throw new Error(`No value for placeholder "${p2.value.name}" was provided`);
...
function mapRelationalRow$1(row, buildQueryResultSelection, mapColumnValue = (value) => value, parseJson = false, parseJsonIfString = false, path) {
  for (const selectionItem of buildQueryResultSelection) {
    if (selectionItem.selection) {
      ...
      if (selectionItem.isArray) {
        for (const item of row[selectionItem.key]) mapRelationalRow$1(item, selectionItem.selection, mapColumnValue, false, parseJsonIfString, currentPath);
        continue;
      }
      mapRelationalRow$1(row[selectionItem.key], selectionItem.selection, mapColumnValue, false, parseJsonIfString, currentPath);
      continue;
    }
  ...
  for (const [target, value] of entries) {
    ...
    switch (target) {
      ...
      case "OR":
        ...
        parts.push(or$1(...value.map((subFilter) => relationsFieldFilterToSQL(column, subFilter))));
        continue;
      case "AND":
        if (!value.length) continue;
...
function relationsFilterToSQL(table, filter, tableRelations = {}, tablesRelations = {}, casing, depth = 0) {
  ...
  for (const [target, value] of entries) {
    ...
    switch (target) {
      ...
      case "OR":
        if (!value?.length) continue;
        parts.push(or$1(...value.map((subFilter) => relationsFilterToSQL(table, subFilter, tableRelations, tablesRelations, casing, depth))));
        continue;
      case "AND":
        if (!value?.length) continue;
...
async function hashQuery(sql2, params) {
  const dataToHash = `${sql2}-${JSON.stringify(params, (_2, v2) => typeof v2 === "bigint" ? `${v2}n` : v2)}`;
...
function extractTablesRelationalConfig(schema2, configHelpers) {
  if (Object.keys(schema2).length === 1 && "default" in schema2 && !is(schema2["default"], Table)) schema2 = schema2["default"];
  const tableNamesMap = {};
  const relationsBuffer = {};
  const tablesConfig = {};
  for (const [key, value] of Object.entries(schema2)) if (is(value, Table)) {
...
function mapRelationalRow(tablesConfig, tableConfig, row, buildQueryResultSelection, mapColumnValue = (value) => value) {
  ...
  for (const [selectionItemIndex, selectionItem] of buildQueryResultSelection.entries()) if (selectionItem.isJson) {
    ...
    result[selectionItem.tsKey] = is(relation, One2) ? subRows && mapRelationalRow(tablesConfig, tablesConfig[selectionItem.relationTableTsKey], subRows, selectionItem.selection, mapColumnValue) : subRows.map((subRow) => mapRelationalRow(tablesConfig, tablesConfig[selectionItem.relationTableTsKey], subRow, selectionItem.selection, mapColumnValue));
  } else {
...
function stringify(hash2, length, startWithLetter = false) {
  let result = "";
  let h = hash2;
  if (startWithLetter) {
...
function preparedStatementName(sql2, params = []) {
  let hash1 = hash(sql2);
  let hash2 = hash(sql2, -559043606);
...
var RelationalQueryBuilder = class {
  ...
  constructor(schema2, table, tableConfig, dialect, session, parseJson, builder = PgRelationalQuery) {
    this.schema = schema2;
    this.table = table;
    this.tableConfig = tableConfig;
    this.dialect = dialect;
...
var PgRelationalQuery = class {
  ...
  constructor(schema2, table, tableConfig, dialect, session, config, mode, parseJson) {
    this.schema = schema2;
    this.table = table;
    this.tableConfig = tableConfig;
    this.dialect = dialect;
  ...
  */
  ...
  _buildRelationalQuery({ fullSchema, schema: schema2, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
    ...
    if (config === true) selection = Object.entries(tableConfig.columns).map(([key, value]) => ({
      ...
      if (config.with) selectedRelations = Object.entries(config.with).filter((entry) => !!entry[1]).map(([tsKey, queryConfig2]) => ({
        tsKey,
  ...
  buildRqbColumn(table, column, key) {
    if (is(column, Column)) {
      ...
      switch (targetType) {
        ...
        case "PgCustomColumn":
          return sql`${column.jsonSelectIdentifier(name, sql, dimensionCnt > 0 ? dimensionCnt : void 0)} as ${sql.identifier(key)}`;
        default:
  ...
  unwrapAllColumns = (table, selection) => {
    return sql.join(Object.entries(table[TableColumns]).map(([k, v2]) => {
      ...
      return this.buildRqbColumn(table, v2, k);
    }), sql`, `);
  };
  buildColumns = (table, selection, config) => config?.columns ? (() => {
    ...
    for (const [k, v2] of entries) {
      ...
      colSelectionMode = colSelectionMode || v2;
      if (v2) {
    ...
    if (colSelectionMode === false) for (const [k, v2] of Object.entries(columnContainer)) {
      if (config.columns[k] === false) continue;
      columnIdentifiers.push(this.buildRqbColumn(table, v2, k));
      selection.push({
        key: k,
        field: v2
      });
  ...
  buildRelationalQuery({ schema: schema2, table, tableConfig, queryConfig: config, relationWhere, mode, errorPath, depth, throughJoin }) {
    const selection = [];
    ...
    const where = params?.where && relationWhere ? and(relationsFilterToSQL(table, params.where, tableConfig.relations, schema2, this.casing), relationWhere) : params?.where ? relationsFilterToSQL(table, params.where, tableConfig.relations, schema2, this.casing) : relationWhere;
    ...
    const joins = params ? (() => {
      ...
      return sql.join(withEntries.map(([k, join]) => {
        ...
        const isSingle2 = is(relation, One$1);
...
var PgAsyncDatabase = class {
  ...
  _query;
  query;
  constructor(dialect, session, relations, schema2, parseRqbJson = false) {
    this.dialect = dialect;
    this.session = session;
    this._ = schema2 ? {
    ...
    this._query = {};
    if (this._.schema) for (const [tableName, columns] of Object.entries(this._.schema)) this._query[tableName] = new _RelationalQueryBuilder(schema2.fullSchema, this._.schema, this._.tableNamesMap, schema2.fullSchema[tableName], columns, dialect, session);
    this.query = {};
...
var I = (r, e) => () => (e || r((e = { exports: {} }).exports, e), e.exports), se = (r, e) => {
  for (var t in e)
...
var Te = (r, e, t) => (t = r != null ? io(ao(r)) : {}, Tn(e || !r || !r.__esModule ? Ce(t, "default", {
  value: r,
  enumerable: true
}) : t, r)), O = (r) => Tn(Ce({}, "__esModule", { value: true }), r);
...
var Kn = I((Le) => {
  ...
  function Hn(r, e, t, n, i) {
    return e = +e, t = t >>> 0, i || Wn(r, e, t, 4), Pe.write(
      r,
      e,
      t,
      n,
...
var Fs = I((hf, Rs) => {
  ...
  Ge(), O(He);
  var bc = tt(), dn = ji(), Sc = Xi(), xc = wt(), Ec = bt(), Ls = ys(), vc = et(), _c = pn(), yn = class yn extends wc {
    constructor(e) {
...
var Vs = I((Ef, $s) => {
  p();
  var Rc = (Ns(), O(Os)), Fc = wt();
  qs();
  var Hs = ge().EventEmitter, Mc = (Ge(), O(He)), Dc = bt(), Gs = Ws(), J = $s.exports = function(r) {
...
a(Pt, "NeonDbError");
var pe = Pt, zs = "transaction() expects an array of queries, or a function returning an array of queries", Wc = ["severity", "code", "detail", "hint", "position", "internalPosition", "internalQuery", "where", "schema", "table", "column", "dataType", "constraint", "file", "line", "routine"];
function Xs(r, {
  arrayMode: e,
  fullResults: t,
  fetchOptions: n,
...
var NeonHttpDriver = class {
  ...
  createSession(relations, schema2) {
    return new NeonHttpSession(this.client, this.dialect, relations ?? {}, schema2, {
...
function wrap(target, token, cb, deep) {
  return new Proxy(target, { get(target2, p2) {
    ...
    if (p2 === "query" || p2 === "_query") return wrap(element, token, cb, true);
    if (p2 === "execute") return new Proxy(element, { apply(target3, thisArg, argArray) {
      return target3.call(thisArg, ...argArray, token);
    } });
...
var NeonHttpDatabase = class extends PgAsyncDatabase {
  ...
  $withAuth(token) {
    return wrap(this, token, (target, p2, res) => {
      if (p2 === "with") return wrap(res, token, (_2, __, res2) => res2);
      return res;
    });
  }
  async batch(batch) {
...
function construct(client, config = {}) {
  ...
  const db2 = new NeonHttpDatabase(dialect, new NeonHttpDriver(client, dialect, {
    logger,
    cache: config.cache
  }).createSession(relations, schema2), relations, schema2);
  db2.$client = client;
  db2.$cache = config.cache;
  if (db2.$cache) db2.$cache["invalidate"] = config.cache?.onMutate;
  return db2;
}
function drizzle(...params) {
  if (typeof params[0] === "string") return construct(Xs(params[0]), params[1]);
  const { connection, client, ...drizzleConfig } = params[0];
  if (client) return construct(client, drizzleConfig);
  if (typeof connection === "object") {
    const { connectionString, ...options } = connection;
    return construct(Xs(connectionString, options), drizzleConfig);
  }
  return construct(Xs(connection), drizzleConfig);
}
(function(_drizzle) {
...
function matchesEncoded(pin, encoded) {
...
async function pinIsConfigured() {
...
async function newSession() {
  ...
  const expiresAt = new Date(Date.now() + SESSION_HOURS * 36e5);
  await db.delete(adminSessions).where(lt(adminSessions.expiresAt, /* @__PURE__ */ new Date()));
...
async function authenticateAdmin(pin) {
  ...
  if (state.lockedUntil > Date.now()) {
    const minutes = Math.ceil((state.lockedUntil - Date.now()) / 6e4);
    throw new AdminError(
  ...
  const valid = fromEnv ? fromEnv === pin.trim() : encoded !== null && matchesEncoded(pin.trim(), encoded);
  if (!valid) {
    const attempts = state.attempts + 1;
    const lockedUntil = attempts >= MAX_ATTEMPTS ? Date.now() + LOCK_MINUTES * 6e4 : 0;
    await writeSetting(
      ATTEMPTS_KEY,
      JSON.stringify({
    ...
    throw new AdminError(
      lockedUntil ? `PIN incorrecto. Acesso bloqueado ${LOCK_MINUTES} min.` : "PIN incorrecto."
    );
  }
...
async function verifySession(token) {
  ...
  const [row] = await db.select({ expiresAt: adminSessions.expiresAt }).from(adminSessions).where(eq(adminSessions.token, token)).limit(1);
  if (!row) return false;
...
async function changePin(token, currentPin, nextPin) {
  ...
  const currentValid = fromEnv ? fromEnv === currentPin.trim() : encoded !== null && matchesEncoded(currentPin.trim(), encoded);
  if (!currentValid) {
...
export {
  AdminError as A,
  asc as a,
  pinIsConfigured as b,
  createServerRpc as c,
  db as d,
  eq as e,
  authenticateAdmin as f,
  endSession as g,
  changePin as h,
  products as p,
  requireAdmin as r,
